...
export default {
  ...
  methods: {
    //更新头像成功时回调的函数，刷新一下页面，显示头像
    onSuccess() {
      this.initAdmin()
    },

    //表单校验,修改密码，清除信息====修改密码时，后端还需要adminId
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          this.putRequest('/admin/pass', this.ruleForm).then(resp => {
            if (resp) {
              //改完密码踢出登录，重新登录，清除sessionStorage里面的内容
    ...
    initAdmin() {
      this.getRequest('/admin/info').then(resp => {
        if (resp) {
          ...
          /**
           * 由于修改完用户信息后可能会导致用户信息不一致，
           * 修改后的用户信息和用户工具栏的信息不一致，
           * 所以需要重新将用户信息存入sessionStorage
           *
           */
